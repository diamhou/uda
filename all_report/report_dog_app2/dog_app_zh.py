...
#     if face_detector(f) == True:
#         dogCount1 += 1
# print("{:.2f}% human faces in human_files".format(humanCount1/len(human_files_short)*100))
# print("{:.2f}% human faces in dog_files".format(dogCount1/len(dog_files_short)*100))

# 优化代码
print("{:.2f}% human faces in human_files".format(np.mean([face_detector(human) for human in human_files_short])*100))
print("{:.2f}% human faces in dog_files".format(np.mean([face_detector(dog) for dog in dog_files_short])*100))
...
# 
# 不合理。我想到两种改善方法：
# 1. 可以使用图像处理技术，比如图像增强、灰度变换等，对模糊的人脸图像进行图像处理
# 2. 让用户不仅提供正视的人脸图像，同时提供侧脸的图像。为我们提供更多的信息。
...
# print(dog_detector(dog_files_short[16]))
# humanCount2 = 0
# dogCount2 = 0
# for f in human_files_short:
#     if dog_detector(f) == True:
#         humanCount2 += 1
# for f in dog_files_short:
#     if dog_detector(f) == True:
#         dogCount2 += 1
# # print(dogCount2)
# print("{:.2f}% human faces in human_files".format(humanCount2/len(human_files_short)*100))
# print("{:.2f}% human faces in dog_files".format(dogCount2/len(dog_files_short)*100))

# 优化代码
print("{:.2f}% human faces in human_files".format(np.mean([dog_detector(human) for human in human_files_short])*100))
print("{:.2f}% human faces in dog_files".format(np.mean([dog_detector(dog) for dog in dog_files_short])*100))
...
# 
# 1. 定义CNN层，检测原始图像。
# 2. 在每个CNN层之后定义MaxPooling最大池化层降低空间维度。
# 3. 为每种类型检测定义133个节点的全连接层。
# 4. 滤波器filter递增，使卷积层更深
# 
# 
# 因此此架构对图像检测可以不依赖于空间相关性
...
train_myResnet50 = bottleneck_features['train']
valid_myResnet50 = bottleneck_features['valid']
test_myResnet50 = bottleneck_features['test']
...
# 
# 步骤：
# 1. 添加GlobalAveragePooling层强化模型特征
# 2. 添加133个节点的全连接层，激活函数选择softmax,
# 
# 原因：
# 
# 1. 这是一种基于转移学习的架构。 我用ResNet-50预训练数据。
# 2. Resnet（残差网络），与传统的顺序网络架构（如AlexNet、OverFeat和VGG）不同，可以让网络在深度增加情况下却不退化。而且Resnet 分类结果要好很多，同时训练速度也非常快。
# 3. VGG, Resnet 等 CNN 网络都是图片分类网络, 都是在imagenet上数据训练出来的。运行很慢。
# 
# 
...
myResnet50_model.add(GlobalAveragePooling2D(input_shape=train_myResnet50.shape[1:]))
...
myResnet50_model.fit(train_myResnet50, train_targets, 
          validation_data=(valid_myResnet50, valid_targets),
...
Resnet50_predictions = [np.argmax(myResnet50_model.predict(np.expand_dims(feature, axis=0))) for feature in test_myResnet50]
...
### TODO: 设计你的算法
### 自由地使用所需的代码单元数吧
def cnn_detect_dogbreed(img_path):
    if face_detector(img_path):
        print("Dog breed for this human: " + Resnet50_predict_breed(img_path))
    elif dog_detector(img_path):
        print("Dog breed for this dog: " + Resnet50_predict_breed(img_path))
    else:
        print("Something Wrong...I can't tell this photo!")
...
images = ['dog01.jpg', 'dog02.jpg', 'dog03.jpg','dog04.jpg', 'human05.jpg', 'human06.jpg']

def get_imagPath(imagName):
    return "./myimag/" + imagName

def show_image(img_path):
    cnn_detect_dogbreed(img_path)
    test_img = plt.imread(img_path)
    plt.imshow(test_img)
    plt.show()
    
for i in images:
    show_image(get_imagPath(i))
